            ...
            case Scheduler of
                ...
                {SchedulerFunction, Arglist} ->
                    ...
                    Chosen=case MaybeProcessInfo of
                        ...
                        { dataJust, #task_info{pid=JustChosen,method=Method} } ->
                            %% Output just for debugging:
                        ...
                        #task_info{pid=JustChosen,method=Method} ->
                            %% Output just for debugging:
                            %io:fwrite("~s ~s ~s", ["[BACKEND] Process for method ", Method, " has been chosen.\n"]),
                            JustChosen
                    end,
...
waiting_for_gc_stop({call, From}, {token,R,TaskState, TaskInfo, ObjectState},
    ...
    NewRunnable = case NewTaskState of
                      runnable -> Run;
                      _ -> gb_sets:del_element(R, Run) end,
    NewWaiting = case NewTaskState of
                     waiting -> gb_sets:add_element(R, Wai);
                     _ -> Wai end,
    NewPolling = case NewTaskState of
